...
TENGWAR_DS_GENERIC_CONF = {
  ...
  :virtual_groups   => TENGWAR_DIACTRITICS_WITH_SIMILAR_PLACEMENT_DS,
  :virtual_triggers => TENGWAR_DIACRITICS_BEARERS
}

TENGWAR_ANNATAR_CONF = {
  :virtuals         => TENGWAR_VIRTUALS_DS.clone.merge(TENGWAR_LIGATURES_ANNATAR),
  :virtual_groups   => TENGWAR_DIACTRITICS_WITH_SIMILAR_PLACEMENT_DS.clone + [["LAMBE"]],
  :virtual_triggers => TENGWAR_DIACRITICS_BEARERS.clone.insert(TENGWAR_DIACRITICS_BEARERS.index("LAMBE")+1,"LAMBE_LIG","LAMBE_NO_LIG")
}
